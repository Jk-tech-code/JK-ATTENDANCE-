  ...
  async function handleTestLocation() {
    ...
    try {
      ...
      if (schoolLat == null || schoolLng == null) {
        setTestMsg(
          'School GPS coordinates not configured. Please set latitude and longitude in the Location Configuration section first.'
        )
        setTesting(false)
        return
      }
      ...
      const dist = haversineDistance(gps.latitude, gps.longitude, schoolLat, schoolLng)
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Configure school settings, GPS geofencing, and attendance time rules"
        />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  {settingsErrors.latitude && (
                    <p className="text-xs text-destructive">{settingsErrors.latitude}</p>
                  )}
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">
                  ...
                  {settingsErrors.longitude && (
                    <p className="text-xs text-destructive">{settingsErrors.longitude}</p>
                  )}
                </div>
              </div>
              <div className="space-y-2">
                ...
                {settingsErrors.allowed_radius_meters && (
                  <p className="text-xs text-destructive">{settingsErrors.allowed_radius_meters}</p>
                )}
                <p className="text-[10px] text-muted-foreground">
                  Current: {settings.school_name ?? 'School'} (
                  {settings.latitude?.toFixed(4) ?? 'N/A'},{' '}
                  {settings.longitude?.toFixed(4) ?? 'N/A'}) &middot;{' '}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {settingsErrors.grace_period_minutes && (
                  <p className="text-xs text-destructive">{settingsErrors.grace_period_minutes}</p>
                )}
                <p className="text-[10px] text-muted-foreground">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">
                  Official Checkout Time <span className="text-destructive">*</span>
                </label>
                <Input
                  type="time"
                ...
                {settingsErrors.checkout_time && (
                  <p className="text-xs text-destructive">{settingsErrors.checkout_time}</p>
                )}
                <p className="text-[10px] text-muted-foreground">
              ...
              <div className="rounded-md border bg-muted/40 p-3 text-xs space-y-1">
                ...
                <p>
                  <span className="text-emerald-600 font-medium">PRESENT</span>: Check-in between
                  reporting start and grace period end
                </p>
                <p>
                  <span className="text-amber-600 font-medium">LATE</span>: Check-in after grace
                  period ends
                </p>
                <p>
                  <span className="text-red-600 font-medium">ABSENT</span>: No check-in recorded
                </p>
                <p>
                  <span className="text-blue-600 font-medium">COMPLETE DAY</span>: Checked in on
                  time and out at/after checkout time
                </p>
                <p>
                  <span className="text-orange-600 font-medium">EARLY DEPARTURE</span>: Checked out
            ...
            <CardContent className="space-y-4">
              ...
              {testLat != null && testLng != null && (
                <div className="space-y-1.5 rounded-md border bg-muted/50 p-3 text-xs">
                  ...
                  <p className="pl-5 text-muted-foreground">
                    School: {settings.latitude?.toFixed(4) ?? 'N/A'},{' '}
                    {settings.longitude?.toFixed(4) ?? 'N/A'}
                  </p>
                  {testMsg && (
                    <p
              ...
              {!testLat && !testing && (
                <p className="text-center text-xs text-muted-foreground">
                  Click "Test GPS Location" to verify your current position against the school
                  geofence.
                </p>
              )}
            </CardContent>
