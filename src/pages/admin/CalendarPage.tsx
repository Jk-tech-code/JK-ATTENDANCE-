...
function getDayColor(day: MonthCalendar['calendar'][0]): string {
  ...
  if (dt === 'working_day') {
    if ((day.present ?? 0) > 0 || (day.late ?? 0) > 0)
      return 'bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800'
    if ((day.absent ?? 0) > 0 && !(day.present ?? 0) && !(day.late ?? 0))
...
export default function CalendarPage() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const csvRows = report.working_days.map(
        (d) => `${d.date},${d.present},${d.late},${d.absent},${d.total}`
      )
      const csv = [
  ...
  const handleAddSave = async () => {
    ...
    try {
      ...
      getMonthCalendar(year, month)
        .then(setData)
        .catch((_err) => toast.error('Failed to refresh calendar'))
    } catch (err) {
    ...
    <>
      ...
      <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {completedDays}/{totalWorking} completed
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <Card>
              ...
              <CardContent>
                  ...
                  <>
                    ...
                    {calendarGrid.map((week, wi) => (
                      <div key={wi} className="grid grid-cols-7 gap-px">
                        {week.map((day, di) => {
                          if (!day)
                            return <div key={di} className="min-h-[72px] rounded-md bg-muted/20" />
                ...
                <div className="flex flex-wrap gap-4 mt-4 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1">
                    <span className="inline-block h-3 w-3 rounded-sm bg-yellow-50 border border-yellow-200" />{' '}
                    Holiday
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="inline-block h-3 w-3 rounded-sm bg-purple-50 border border-purple-200" />{' '}
                    Event
                  </span>
                  <span className="flex items-center gap-1">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <p className="text-sm text-muted-foreground capitalize">
                        {dayDetail.day_type.replace('_', ' ')}
                        {dayDetail.title ? ` - ${dayDetail.title}` : ''}
                      </p>
                    </CardHeader>
        ...
        <Dialog
          ...
          title={addForm.day_type === 'holiday' ? 'Add Holiday' : 'Add Event'}
        >
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>
              ...
              <Input
                type="date"
                value={addForm.calendar_date}
                onChange={(e) => setAddForm({ ...addForm, calendar_date: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              <Label>
              ...
              <Input
                ...
                placeholder={
                  addForm.day_type === 'holiday' ? 'e.g., National Holiday' : 'e.g., Sports Day'
                }
              />
            </div>
            <div className="space-y-2">
