...
import { useDailyReport, useMonthlyReport, useAiAnalysis } from '@/hooks/useReports'
...
export default function ReportsPage() {
  ...
  const { data: daily, isLoading: dailyLoading, error: dailyError } = useDailyReport(today)
  const {
    data: monthly,
  ...
  const handleExport = async (format: 'csv' | 'xlsx' | 'pdf') => {
    try {
      ...
      const fetched =
        records.length === 0 || records[0].attendance_date < endDate
          ? (
    ...
    <>
      ...
      <div className="space-y-6">
              ...
              {daily ? (
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-lg">{daily.attendance_rate}%</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{daily.avg_check_in_time}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Avg Working Min</span>
                    <span>{daily.avg_working_minutes}</span>
                  </div>
                </div>
              ) : (
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-lg">
                      {monthly.summary.attendance_percentage}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Present Days</span>
                    <span>{monthly.summary.present_days}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Late Days</span>
                    <span>{monthly.summary.late_days}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Absent Days</span>
                    <span>{monthly.summary.absent_days}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Avg Working</span>
                    <span>{monthly.summary.avg_working_hours} hrs</span>
                  </div>
                </div>
              ) : (
                <p className="text-muted-foreground">No data</p>
        ...
        {monthly && monthly.teachers && monthly.teachers.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Per-Teacher Attendance</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        {monthly && monthly.teachers && monthly.teachers.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Present vs Absent (Monthly)</CardTitle>
            </CardHeader>
            <CardContent>
            ...
            <CardContent className="space-y-6">
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-lg border bg-card p-4">
                  ...
                  <p
                    className={`text-2xl font-bold ${(aiData.insights.summary.attendance_rate ?? 0) >= 90 ? 'text-green-600' : (aiData.insights.summary.attendance_rate ?? 0) >= 75 ? 'text-yellow-600' : 'text-red-600'}`}
                  >
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {aiData.insights.suggestions.map((s, i) => (
                      <li key={i} className="rounded-md bg-muted/50 px-3 py-2 text-sm">
                        {s}
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {aiData.ai_generated.recommendations.map((r, i) => (
                      <li
                        key={i}
                        className="rounded-md bg-primary/5 border border-primary/10 px-3 py-2 text-sm"
                      >
                        {r}
                      </li>
                    ))}
