  ...
  try {
    ...
    if (!callerIsAdmin) {
      const { data: callerTeacher } = await supabase
        .from('teachers')
        .select('id')
        .or(`id.eq.${auth.user!.id},user_id.eq.${auth.user!.id},auth_user_id.eq.${auth.user!.id}`)
        .maybeSingle()
    ...
    const reportingStart =
      settings?.reporting_start_time ?? settings?.default_reporting_time ?? '07:00'
    ...
    if (earlyDepartureMinutes !== null) result.early_departure_minutes = earlyDepartureMinutes
