...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
      ...
      .map(([_teacherId, agg], index) => ({
        ...
        avg_working_minutes:
          agg.workingMinutes.length > 0
            ...
            : 0,
        attendance_rate:
          agg.total > 0 ? Math.round(((agg.present + agg.late) / agg.total) * 100) : 0,
      }))
    ...
    const overallPresent =
      attendance?.filter((a) => ['present', 'checked_out'].includes(a.status ?? '')).length ?? 0
    const totalRecords = attendance?.length ?? 0

    const workingMins =
      attendance?.map((a) => a.working_minutes).filter((m): m is number => m !== null) ?? []
    ...
    const anonymizedInsights = {
      ...
      summary: {
        ...
        attendance_rate:
          totalRecords > 0 ? Math.round(((overallPresent + overallLate) / totalRecords) * 100) : 0,
            ...
            messages: [
              {
                role: 'system',
                content:
                  'You are an attendance analytics expert for schools. All data is anonymized.',
              },
              {
                role: 'user',
